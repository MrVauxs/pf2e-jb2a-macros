...
pf2eAnimations.createIfMissingDummy = async function createIfMissingDummy() {
  let message = `PF2e Animations | ${pf2eAnimations.localize(
    "pf2e-jb2a-macros.notifications.noDummy"
  )}`;
  let npcFolder = game.folders.get(
    game.settings.get("pf2e-jb2a-macros", "dummyNPCId-folder")
  );

  if (!npcFolder) {
    npcFolder = await Folder.create({
      name: "PF2e Animations Dummy NPCs",
      type: "Actor",
      parent: null,
    });
  ...
  let npcActor1 = game.actors.get(
    game.settings.get("pf2e-jb2a-macros", "dummyNPCId-tiny")
  );
  let npcActor2 = game.actors.get(
    game.settings.get("pf2e-jb2a-macros", "dummyNPCId-medium")
  );
  let npcActor3 = game.actors.get(
    game.settings.get("pf2e-jb2a-macros", "dummyNPCId-large")
  );
  let npcActor4 = game.actors.get(
    game.settings.get("pf2e-jb2a-macros", "dummyNPCId-huge")
  );
  let npcActor5 = game.actors.get(
    game.settings.get("pf2e-jb2a-macros", "dummyNPCId-garg")
  );
  if (!(npcActor1 && npcActor2 && npcActor3 && npcActor4 && npcActor5)) {
    message += ` ${pf2eAnimations.localize(
      "pf2e-jb2a-macros.notifications.creatingDummy"
    )} `;
 ...
 */
pf2eAnimations.playerSummons = async function playerSummons({
  args = [],
  importedActor = {},
  spawnArgs = {},
}) {
  ...
  if (!Object.keys(importedActor).length) {
    ...
    packs = packs.map((pack) => {
      return {
        ...pack,
        traits: pack.traits.concat(
          pack.actorSize,
          pf2eAnimations.alignmentStringToTraits(pack.alignment)
        ),
      };
    });
    ...
    let sortedHow = {
      type: "info",
      label: pf2eAnimations.localize(
        "pf2e-jb2a-macros.macro.summoning.player.label"
      ),
    };
    ...
    if (args && args[2]?.length) {
      ...
      if (args[2].find((x) => x.includes("unique"))) {
        ...
        switch (unique) {
          case "lesser-servitor": {
            // spell level
            let multiplier = -1;
            if (args[0].flags.pf2e.origin.castLevel >= 2) multiplier = 1;
            if (args[0].flags.pf2e.origin.castLevel >= 3) multiplier = 2;
            if (args[0].flags.pf2e.origin.castLevel >= 4) multiplier = 3;
            packs = packs.filter((x) => x.level <= multiplier);
            packs = packs.filter(
              (x) =>
                // celestial, monitor, or fiend
                ["celestial", "monitor", "fiend"].some((traitOr) =>
                  x.traits.includes(traitOr)
                ) ||
            ...
            alignment =
              args[1].sourceToken.actor?.deity?.system?.alignment?.own ??
              args[1].sourceToken.actor?.system.details?.alignment?.value;
            reverseAlignment = pf2eAnimations.alignmentStringToTraits(
              alignment,
              true
            );
            !reverseAlignment.length
              ? packs
              : (packs = packs.filter((x) =>
                  pf2eAnimations
                    .alignmentStringToTraits(alignment, true)
                    .some((t) => !x.traits.includes(t))
                ));
      ...
      const summon = args[2].includes("summon-spell-")
        ? args[2]
            .find((x) => x.includes("summon-spell-"))
            ?.replace("summon-spell-", "")
            .split("-")
        : args[2].includes("summon-spell");
      const traitsOr = args[2]
        .find((x) => x.includes("trait-or-"))
        ?.replace("trait-or-", "")
        .split("-");
      const traitsAnd = args[2]
      ...
      const uncommon =
        args[2].find(
          (x) =>
            x.includes("uncommon") || x.includes("rare") || x.includes("unique")
        ) ?? game.settings.get("pf2e-jb2a-macros", "allowUncommonSummons");
      const exactLevel = args[2]
      ...
      const level = args[2]
        ...
        .split("-");
      const hasImage =
        game.settings.get("pf2e-jb2a-macros", "onlyImageSummons") ||
        args[2].find((x) => x.includes("has-image"));
      const source = args[2]
        .find((x) => x.includes("source"))
        ?.replace("source-", "")
        .split("|")
        .map((x) => x.trim()); // separate by | for multiple sources
      const name = args[2]
      ...
      randomAmount = args[2]
        .find((x) => x.includes("random-amount"))
        ?.replace("random-amount-", "")
        .split("-");

      if ([exactLevel, level, summon].filter(Boolean).length > 1)
        return ui.notifications.error(
          game.i18n.format("pf2e-jb2a-macros.notifications.tooManyArgs", {
            issues: "exactLevel, level, summon-spell",
          })
        );
      ...
      if (summon) {
        ...
        if (args[0].flags.pf2e.origin.castLevel >= 3) multiplier = 2;
        if (args[0].flags.pf2e.origin.castLevel >= 4) multiplier = 3;
        if (args[0].flags.pf2e.origin.castLevel >= 5) multiplier = 5;
        if (args[0].flags.pf2e.origin.castLevel >= 6) multiplier = 7;
        if (args[0].flags.pf2e.origin.castLevel >= 7) multiplier = 9;
        if (args[0].flags.pf2e.origin.castLevel >= 8) multiplier = 11;
        if (args[0].flags.pf2e.origin.castLevel >= 9) multiplier = 13;
        if (args[0].flags.pf2e.origin.castLevel >= 10) multiplier = 15;
      ...
      if (level) {
        ...
        packs = packs.filter(
          (x) => x.level >= Number(level[0].replace("~", "-"))
        );
        // level equal or less filter
        if (level[1])
          packs = packs.filter(
            (x) => x.level <= Number(level[1].replace("~", "-"))
          );
      }
      if (exactLevel) {
        // level equal filter
        packs = packs.filter(
          (x) => x.level == Number(exactLevel.replace("~", "-"))
        );
      }
      if (hasImage) {
        // non default icons
        packs = packs.filter(
          (x) => !x.img.includes("systems/pf2e/icons/default-icons")
        );
      }
      if (traitsOr) {
        // traits OR filter
        packs = packs.filter((x) =>
          traitsOr.some((traitOr) => x.traits.includes(traitOr))
        );
      }
      if (traitsAnd) {
        // traits AND filter
        packs = packs.filter((x) =>
          traitsAnd.every((traitAnd) => x.traits.includes(traitAnd))
        );
      }
      if (!uncommon) {
        // common rarity
      ...
      sortedHow.label = [
        `<p>${pf2eAnimations.localize(
          "pf2e-jb2a-macros.macro.summoning.player.sorted"
        )}</p>`,
        args[2].length
          ? [
              unique
              ...
              summon
                ? `<p>${game.i18n.format(
                    "pf2e-jb2a-macros.macro.summoning.player.summonArg",
                    {
                      multiplier: multiplier,
                      spellLevel: args[0].flags.pf2e.origin.castLevel,
                    }
                  )}</p>`
                : "",
              level
              ...
              uncommon
                ? `<p>${pf2eAnimations.localize(
                    "pf2e-jb2a-macros.macro.summoning.player.uncommonArg"
                  )}</p>`
                : "",
              hasImage
                ? `<p>${pf2eAnimations.localize(
                    "pf2e-jb2a-macros.macro.summoning.player.hasImageArg"
                  )}</p>`
                : "",
              source
                ? `<p>${game.i18n.format(
                    "pf2e-jb2a-macros.macro.summoning.player.sourceArg",
                    { sources: source.join(", ") }
                  )}</p>`
                : "",
            ].join("")
    ...
    let inputs = [sortedHow];

    if (packs.length === 0)
      packs.push({
        level: 420,
        name: pf2eAnimations.localize(
          "pf2e-jb2a-macros.macro.summoning.player.nothingFound"
        ),
      });
    ...
    if (!randomCreature) {
      inputs.push({
        type: "select",
        label: pf2eAnimations.localize(
          "pf2e-jb2a-macros.macro.summoning.player.creature"
        ),
        options: packs.map((x) => {
          return {
            value: x.name,
            html: `${
              x.name
            } <span style="text-align:center;">| ${game.i18n.format(
              "pf2e-jb2a-macros.macro.summoning.player.level",
              { level: x.level }
            )}</span>`,
          };
        }),
    ...
    if (!randomAmount) {
      inputs.push({
        type: "number",
        label: pf2eAnimations.localize(
          "pf2e-jb2a-macros.macro.summoning.player.amount"
        ),
        options: 1,
      });
    }
    ...
    if (!(randomAmount && randomCreature)) {
      options = await warpgate.menu(
        ...
        {
          title: pf2eAnimations.localize(
            "pf2e-jb2a-macros.macro.summoning.player.title"
          ),
        }
      );
    ...
    (spawnArgs.options ??= {}).duplicates = randomAmount
      ? Sequencer.Helpers.random_int_between(randomAmount[0], randomAmount[1])
      : options.inputs[randomCreature ? 1 : 2];
  }

  spawnArgs.origins = {
    tokenUuid: tokenD.data.uuid,
    itemUuid: args?.[1]?.itemUuid,
  ...
  spawnArgs.updates = {
    token: importedToken,
    actor: importedActor.data.toObject(),
  };
  spawnArgs.updates.actor.system.details.alliance =
    tokenD.actor.system.details.alliance;
  ...
  spawnArgs.actorName = `Dummy NPC (${
    spawnArgs.updates.token.actor.size === "sm"
      ? "med"
      : spawnArgs.updates.token.actor.size
  })`;

  const crosshairConfig = {
    label: importedToken.name,
    interval:
      importedToken.height < 1 ? 4 : importedToken.height % 2 === 0 ? 1 : -1,
    lockSize: true,
    drawOutline: false,
    drawIcon: false,
  };
...
pf2eAnimations.askGMforSummon = async function askGMforSummon(args) {
  if (!warpgate.util.isFirstGM())
    return pf2eAnimations.debug("Not the first GM, ignoring request!", args);
  ...
  let template = await canvas.templates.get(
    args.location.id ?? args.location._id
  );

  if (!args.callbacks) args.callbacks = {};

  if (args?.callbacks?.pre)
    ui.notifications.error(
      "PF2e Animations | You are providing a callbacks.pre function to the summoning macro. Please note it is going to be overriden in the module."
    );
  ...
  if (args?.callbacks?.post)
    ui.notifications.error(
      "PF2e Animations | You are providing a callbacks.post function to the summoning macro. Please note it is going to be overriden in the module."
    );

  args.callbacks.post = async (
    location,
    spawnedTokenDoc,
    updates,
    iteration
    ...
    let items = (args.options.controllingActor.items || []).filter((i) =>
      i.name.includes("Summoning Animation Template")
    );
    ...
    let item =
      items.find((i) =>
        i.name.includes(
          `Summoning Animation Template (${
            args.actorName?.actor?.name ?? args.actorName
          })`
        )
      ) ??
      items.find((i) =>
        i.name.includes(
          `Summoning Animation Template (${args?.updates?.token?.name})`
        )
      ) ??
      items.find((i) =>
        i.name.includes(
          `Summoning Animation Template (${args.origins.itemName})`
        )
      ) ??
    ...
    await AutomatedAnimations.playAnimation(
      token,
      item ?? {
        name: `Summoning Animation Template (${args.origins.itemName})`,
      },
      { targets: [spawnedTokenDoc.object] }
    );
    if (updates.token.flags["pf2e-jb2a-macros"]?.scrollingText)
      await game.settings.set("core", "scrollingStatusText", true);
    if (updates.token.flags["pf2e-jb2a-macros"]?.bloodsplatter)
      await game.settings.set("splatter", "enableBloodsplatter", true);
    if (updates.token.flags["pf2e-jb2a-macros"]?.tokenmold)
      await game.settings.set("Token-Mold", "everyone", {
        ...game.settings.get("Token-Mold", "everyone"),
        ...{ name: { use: true } },
      });
  ...
  new Dialog({
    ...
    content: game.i18n.format("pf2e-jb2a-macros.macro.summoning.gm.content", {
      actorName:
        args?.updates?.token?.name ??
        args.actorName?.actor?.name ??
        args.actorName,
      amount: args?.options?.duplicates ?? "1",
      user: args.userId
        ? game.users.find((x) => x.id === args.userId).name
        : pf2eAnimations.localize(
            "pf2e-jb2a-macros.macro.summoning.gm.unknownUser"
          ),
    }),
    buttons: {
      button1: {
        label: "Accept",
        callback: async () => {
          if (args.options && args.updates && args.updates.token)
            args.updates.token.actorData = { ownership: { [args.userId]: 3 } };

          if (args?.updates?.actor?.token)
      ...
      button2: {
        ...
        callback: async () => {
          ui.notifications.info(
            pf2eAnimations.localize(
              "pf2e-jb2a-macros.macro.summoning.gm.declined"
            )
          );
          await template.document.delete();
        },
