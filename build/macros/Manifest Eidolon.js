...
let eidolons = game.actors.filter(v =>
    (
        v.class?.slug === "eidolon"
        || v.class?.flags?.core?.sourceId === "Item.4yrittMrVNhR2woZ"
        || v.class?.flags?.core?.sourceId === "Compendium.pf2e-animal-companions.AC-Features.xPn27nNxcLOByTXJ"
        || v.class?.flags?.core?.sourceId === "Compendium.pf2e-animal-companions.AC-Features.Item.xPn27nNxcLOByTXJ"
    )
    && v.isOwner
)
...
foundrySummons.openMenu({
    creatures: eidolons.map(eido => new CONFIG.FoundrySummons.docWrapperClasses.DocWrapper(eido)),
    options: {
        autoPick: true,
